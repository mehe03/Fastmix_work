...
cat( "\n\n with MSE: ", mean((beta_mean-c(1,1,2,-1))^2 ))
cat("\n\n The beta estimate: \n")
cat(beta_mean, "\n\n with MSE: ", mean((beta_mean-c(1,2,-1,2))^2 ))
...
cat("\n\n with MSE: ", mean((beta_mean_lmer-c(1,1,2,-1))^2 ))
cat("\n\n The beta estimate: \n")
...
run_fastmix_on_dataset_outlier <- function(df, RAND_LIST = c(1,2,3,4), iter = 50) {
  ...
  df$X00 <- 1
  df_simple <- df_simple[, c(names(df_simple)[1:(ncol(df_simple)-2)], "X00", names(df_simple)[ncol(df_simple)-1])]
  Des <- df[, c(1, 4, 5, 6, 7)]
  y_values <- df[3]
...
mse_lmer_all <- sapply(seq_along(lmer_results_list), function(i) {
  ...
  true_gamma <- simple_re_list_outlier[[i]][ , 
