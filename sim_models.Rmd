...
for(i in 1:iter) {
  ...
  gamma_i <- Sigma_gamma_init %*% t(Zi) %*% Vi_inv %*% resid_wls[idx,1]
...
#   Zi <- as.matrix(DZ[idx, -1])
#   
#   #Sigma_Yi <- Zi %*% Sigma_gamma %*% t(Zi) + sigma2 * diag(nrow(Zi))
...
# gamma_fun <- function(i, subject_results, Y, Des, beta, Sigma_gamma, sigma2) {
#   Zi  <- subject_results[[i]]$Zi
#   idx <- subject_results[[i]]$idx
#   
#   resid_i <- Y[idx] - as.matrix(Des[idx, -1]) %*% beta
#   
#   wood_1 <- rsolve(
#     rsolve(Sigma_gamma) + (1 / sigma2) * (t(Zi) %*% Zi),
#     min.cond.num = 1e-6
#   )
#   
#   wood_2 <- (1 / sigma2^2) * Zi %*% wood_1 %*% t(Zi)
#   Vi_inv <- (1 / sigma2) * diag(nrow(Zi)) - wood_2
#   
...
fastmix_iterative <- function(Y, Des, random = "all", iter = 100) {
  ...
  for(it in 1:iter) {
    ...
    # Sigma_gamma_new <-  damp_a * Sigma_gamma_new +  (1 - damp_a) * Sigma_gamma_init
      Sigma_gamma_new <- Reduce("+", lapply(gamma_out, `[[`, "gamma_cov_i")) / m
...
saveRDS(results_list, "RDS_RESULTS_SIMPLE_ONE_COV.RDS")
...
```{r fin_results}
true_beta = c(2,1)
true_sigma_gamma = diag(c(0.5,0.25)^2)
## FASTMIX
...
# COMMENTED OUT EVERYTHING FOR CHECKING
# 
# 
# # ------------------------------
# # 1. Function to compute average error per iteration for dataset
...
true_beta <- c(1, 2, -1, 2)
true_sigma <- diag(c(0.25^2, 0.75^2, 0.5^2, 1^2))

for(i in 900){
  
  # Calculate RMSE for beta
  rmse_beta <- sapply(results_list[[i]]$beta_history, function(mat) {
    sqrt(mean((as.numeric(mat[,1]) - true_beta)^2))
  })
  
  # Calculate RMSE for Sigma_gamma
  rmse_sigma <- sapply(results_list[[i]]$Sigma_gamma_history, function(S) {
    sqrt(mean((S - true_sigma)^2))
  })
  
  # Calculate tolerance (difference between consecutive Sigma_gamma matrices)
  tolerance <- sapply(2:length(results_list[[i]]$Sigma_gamma_history), function(j) {
    sqrt(mean((results_list[[i]]$Sigma_gamma_history[[j]] - 
               results_list[[i]]$Sigma_gamma_history[[j-1]])^2))
  })
  
  # Set up plot with appropriate y-axis range
  y_range <- range(c(rmse_beta, rmse_sigma, c(NA, tolerance)), na.rm = TRUE)
  ...
  plot(
    ...
    xlab = "Iteration",
    ylab = "RMSE / Tolerance",
    main = paste("Convergence Metrics - Dataset", i),
    ylim = y_range
  )
  
  # Add Sigma RMSE
  lines(rmse_sigma, lwd = 2, col = "red")
  
  # Add tolerance (starts at iteration 2)
  lines(2:length(results_list[[i]]$Sigma_gamma_history), tolerance, lwd = 2, col = "darkgreen")
  
  # Add legend
  legend("topright", 
         legend = c("Beta RMSE", "Sigma RMSE", "Sigma Tolerance"),
...
results_list[[900]]$Sigma_gamma_history
